<script lang="ts">
	...
	import WorkspacePill from '$lib/WorkspacePill.svelte';
...
<div class="index-page">
	<header class="site-header">
		<div class="header-inner">
			<BrandLogo />
			<WorkspacePill workspaces={data.workspaces} current={data.currentWs} />
			<Search />
			<a href="/new?ws={data.currentWs.id}" class="new-btn" aria-label="Novo documento">
				<span class="new-btn-text">+ Novo</span>
				<svg class="new-btn-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" aria-hidden="true">
	...
	<main class="index-main">
		{#if data.docs.length === 0}
			<div class="empty-state">
				<p>Nenhum documento ainda em <strong>{data.currentWs.name}</strong>.</p>
				<a href="/new?ws={data.currentWs.id}">Crie seu primeiro documento →</a>
			</div>
		{:else}
							...
							{#each filtered as doc}
								<li class="doc-item">
									<a href="/w/{data.currentWs.id}/{doc.slug}" class="doc-link">
										...
										<span class="doc-meta">{doc.slug} &middot; {formatDate(doc.date ?? doc.mtime)}</span>
									</a>
								</li>
							{/each}
						</ul>
...
<style>
	...
	.new-btn:hover { background: var(--brand); }
	.new-btn-icon { display: none; width: 16px; height: 16px; }
